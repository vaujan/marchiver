...
import { Link, Image, Search, Plus, SquarePen, ChevronDown } from 'lucide-react'
...
const getEntryDescription = (entry: Entry): string => {
  const tagText = entry.tags.length > 0
...
const ItemList: React.FC<ItemListProps> = ({
  ...
  const handleAddUrl = async () => {
    ...
    if (urlCaptureScreenshot) {
      const result = await window.electronAPI.captureScreenshot(urlValue.trim())
      if (result.success) {
  ...
  const handlePickImage = async () => {
    ...
    if (path) {
      setImagePath(path)
      if (!imageTitle.trim()) {
        const filename = path.split('/').pop() || path.split('\\').pop() || 'Imported Image'
  ...
  const toggleTag = (tagId: number, currentIds: number[], setIds: (ids: number[]) => void) => {
    ...
    <div className="flex flex-wrap gap-1.5">
      {tags.map((tag) => {
        ...
        return (
          ...
          >
            <span style={{ color: isSelected ? undefined : tag.color }}>#</span>{' '}
            {tag.name}
          </Badge>
        )
      })}
    ...
    <div className="w-[380px] min-w-[380px] bg-card border-r border-border flex flex-col">
      ...
      {searchMode && (
        <div className="px-4 py-2 border-b border-border">
          <InputGroup>
            <Search className="absolute left-2 top-1/2 -translate-y-1/2 opacity-50 pointer-events-none size-4" />
            <InputGroupInput
              placeholder="Search..."
              value={searchQuery}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <FieldGroup>
            ...
            <Field>
              ...
              <Select value={urlFolderId} onValueChange={(val) => { if (val) setUrlFolderId(val) }}>
                <SelectTrigger>
            ...
            <Field orientation="horizontal">
              <Checkbox
                ...
                onCheckedChange={(checked) => setUrlCaptureScreenshot(checked === true)}
              />
      ...
      <Dialog open={imageDialogOpen} onOpenChange={setImageDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <FieldGroup>
            ...
            <Field>
              <FieldLabel>Folder</FieldLabel>
              <Select value={imageFolderId} onValueChange={(val) => { if (val) setImageFolderId(val) }}>
                <SelectTrigger>
                  <SelectValue placeholder="Select folder" />
                </SelectTrigger>
                <SelectContent>
            ...
            <Field>
              <FieldLabel>Tags</FieldLabel>
              <TagSelector selectedIds={imageTagIds} onChange={setImageTagIds} />
            </Field>
          </FieldGroup>
          <DialogFooter>
            <Button
          ...
          {entries.map((entry) => {
            ...
            return (
              <div
                ...
                className="group cursor-pointer border border-transparent bg-background p-4 transition-all hover:bg-muted/50 data-[selected=true]:border-r-[3px] data-[selected=true]:border-r-primary data-[selected=true]:bg-accent"
